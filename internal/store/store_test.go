...
func TestMigrationsIdempotent(t *testing.T) {
	...
	if count != 3 {
		t.Fatalf("migration count after first open: got %d, want 3", count)
	}
	...
	if count != 3 {
		t.Errorf("migration count after second open: got %d, want 3 (re-applied)", count)
	}
}
...
func TestMigrationsExpectedColumns(t *testing.T) {
	...
	cases := []struct {
		...
		{"admin_audit_logs", []string{"id", "event", "api_key_id", "remote_ip", "status_code", "created_at"}},
	}

...
func TestMigrationsCreateExpectedTables(t *testing.T) {
	s := openTestStore(t)
	want := []string{"admin_audit_logs", "api_keys", "node_enrollment_tokens", "node_model_catalogs", "nodes", "request_audit_logs", "schema_migrations"}
...
func TestMigrationsIdempotent(t *testing.T) {
	...
	if count != 4 {
		t.Fatalf("migration count after first open: got %d, want 4", count)
	}
	...
	if count != 4 {
		t.Errorf("migration count after second open: got %d, want 4 (re-applied)", count)
	}
}
...
func TestMigrationsExpectedColumns(t *testing.T) {
	...
	cases := []struct {
		...
		{"admin_audit_logs", []string{"id", "event", "api_key_id", "detail", "remote_ip", "status_code", "created_at"}},
	}
